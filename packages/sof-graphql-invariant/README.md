...
```shell
yarn add @caremesh/sof-graphql-invariant
```
...
```javascript
const scopeInvariant = require('@caremesh/sof-graphql-invariant');
...
`@caremesh/sof-graphql-invariant` exports a single function which takes two arguments. One is a set of options and the other is a resolver function.
