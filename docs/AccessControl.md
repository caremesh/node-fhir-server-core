...
```javascript
const FHIRServer = require('@caremesh/node-fhir-server-core');
...
If you are not familiar with writing passport strategies, you can use [`@caremesh/sof-strategy`](https://github.com/Asymmetrik/node-fhir-server-core/tree/master/packages/sof-strategy). To use this, create a file that looks like this:
...
You will also need to add `@caremesh/sof-strategy` as a dependency by running:
...
```javascript
const { Server } = require('@caremesh/node-fhir-server-core');
...
```javascript
const { initialize, loggers, constants } = require('@caremesh/node-fhir-server-core');
